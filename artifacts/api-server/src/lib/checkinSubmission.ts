...
import type { SubmitCheckinBody, ProgramSchedule } from "@workspace/api-zod";
import { generateStructured } from "./anthropic";
import { checkinAdjustmentOutputSchema } from "./programSchema";
import { checkInEngineKnowledge } from "./knowledge";
import { buildCheckinEvidence } from "./checkinData";
import { computeSessionAdherence, MISSED_REASON_TEXT } from "./sessionAdherence";
...
import { PHASE_TEMPLATES, resolvePhaseProgression, effectivePhase } from "./phaseTemplate";
import { defaultFixedSchedule, remapSchedule } from "./programSchedule";
import { serializeCheckin, serializeProgram, type SerializedProgram } from "./serializers";
...
import { todayDateString } from "./dateWindow";

// Weekly check-in submission + AI reprogramming, shared by the blocking
// POST /checkins and the job endpoint POST /checkins/jobs. Both call prepare +
// run, so the two paths cannot drift apart.
...
// lineage. Shared by GET /checkins/adherence and the check-in submission so the
// form and the prompt always measure against the same program.
export function currentAiProgram(userId: string) {
...
export function recentWorkoutLogs(userId: string) {
...
// plus the client's own logged data. Prompt-cached like program generation, and
// built once at module load for the same reason.
...
export type CheckinSubmissionResult = {
  checkin: ReturnType<typeof serializeCheckin>;
  aiMessage: string;
  updatedProgram: SerializedProgram;
};

// The cheap half: gathers the evidence and rejects a check-in there is no
// program to adjust for. Nothing is written and Claude is not called until
// `run()`, which records the check-in first and then reprograms.
export async function prepareCheckinSubmission(
  userId: string,
  input: ReturnType<typeof SubmitCheckinBody.parse>,
): Promise<PreparedAiWork<CheckinSubmissionResult>> {
  ...
  if (!currentProgram) {
    return { ok: false, status: 400, error: "No program to adjust" };
  }

  const run = async (): Promise<CheckinSubmissionResult> => {
    ...
    const missedSessionReason =
      adherence.loggedSessions < adherence.plannedSessions ? (input.missedSessionReason ?? null) : null;
...
- Sleep quality: ${input.sleep}/5
- Hunger/appetite: ${input.hungerAppetite}/5
- Off days deviating from calorie intake without logging: ${input.offDayDeviation ? "YES - calorie/bodyweight data unreliable this week" : "no"}
- Soreness going into sessions: ${input.soreness}
- Exercise issues (connection / joint / muscle pain): ${input.exerciseIssues || "none reported"}
- What went well: ${input.wentWell || "not provided"}
- What did not go well / to improve: ${input.didntGoWell || "not provided"}
- Decline in sleep duration or quality, and why: ${input.sleepDecline || "not provided"}
- Digestion issues: ${input.digestionIssues || "none reported"}
...
${evidence.text}`;
    ...
    const raw = await generateStructured({
      purpose: "check-in adjustment",
      system: systemInstructions,
      prompt: userPrompt,
      schema: checkinAdjustmentOutputSchema,
    });
